...
public class OrderServiceImpl implements OrderService{
    ...
    private static String SERVER_NAME="store-client";
    ...
    @HystrixCommand(
            fallbackMethod = "orderFallback",//降级处理策略（可以继续降级/返回提示/存根。。。）
            commandKey = "order.do",
            groupKey = "order-group",//相同groupkey有相同的隔离策略
            commandProperties ={
                    @HystrixProperty(name = "execution.isolation.semaphore.maxConcurrentRequests",value = "10"),//并发量
            ...
            threadPoolProperties = {
                @HystrixProperty(name = "coreSize",value = "10")   //线程池策略是线程数大小
            }
    )
    @Override
    public String order(int id) {
        String re = restTemplate.getForObject("http://" + SERVER_NAME + "/order.do", String.class);
//            int i=10/0;
        return re;
    }

    @Override
    public ResponseEntity orderDel(Map<String, Object> param) {
        ResponseEntity<String> responseEntity = restTemplate.postForEntity("http://" + SERVER_NAME + "/orderDel.do", param, String.class);

        return responseEntity;
    }

    public String orderFallback(int id){
        System.out.println("===order.do===fallback()==="+id);
        return "error";
    }
}
